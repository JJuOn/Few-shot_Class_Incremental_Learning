...
#         self.novel_embeds = torch.cat((self.novel_embeds, new_novel_embeds), 0)
    ...
    def update_base_embeds(self, prev_vocab_novel):
        opt = self.opt
        dim = opt.word_embed_size
        embed_pth = os.path.join(opt.word_embed_path, "{0}_dim{1}.pickle".format(opt.dataset, dim))
        prev_novel_embeds = get_embeds(embed_pth, prev_vocab_novel).float().cuda()
        self.prev_novel_embeds = prev_novel_embeds
        if opt.glove:
            self.prev_novel_embeds = self.prev_novel_embeds[:, :300]
        self.base_embeds = torch.cat([self.base_embeds, self.prev_novel_embeds], dim=0)
